...
content.route('/grewtale/media/v1/photo/:name').get((_req, res) => {
  ...
  if (
    ...
    query.en_session === undefined ||
    query.u_id === undefined ||
    query.en_date === undefined ||
    query.en_name === undefined
  ) {
    throw 'Bad URL hash.';
  } else {
    const r = checkingContentUrl(
      query.en_session,
      query.en_date,
      query.en_name,
      _req.params.name
...
content.route('/grewtale/media/v1/thumbnails/:name').get((_req, res) => {
  ...
  if (
    ...
    if (r === true) {
      res.sendFile(
        __dirname + `/media/grewtales/thumbnails/${_req.params.name}`
      );
    } else {
      throw r;
    }
