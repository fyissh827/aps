...
try {
 _final = await (await con()).query(`UPDATE feedback SET reply = '1' WHERE id = ${access.payload}`)}
	  catch(e){
		_final = 'Bug'
	  }
	  if(_final == undefined){
...
try {
 _final = await (await con()).query(`INSERT INTO feedback(name , email, message, submitted_at) VALUE('${access.payload._n}', '${access.payload._e}', '${access.payload._m}', now())`)}
	  catch(e){
		_final = 'Bug'
	  }
	  if(_final == undefined){
