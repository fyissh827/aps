...
async function home_g(e) {
	...
	return await _output.response(s), {
		output: s
	}
}
...
async function getContent(e) {
	var t = [],
		...
		l = [];
	try {
		const [r, s] = await con.query(`SELECT content from grewtales where id = '${e.id}'`);
		t = r
	} catch (e) {
