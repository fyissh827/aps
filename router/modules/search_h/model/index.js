...
try {
 const [row, fields] = await con.execute(`SELECT * FROM search_history WHERE user_id = "${payload.user_id}" ORDER BY id DESC LIMIT ${payload.items} OFFSET ${payload.offset}`);
  _final = row;
 }
	  catch(e){
		_final = ''
