...
const { GlobalComment } = require('../../models/index.js');
...
async function _delete(payload) {
  ...
  try {
    const [row, err] = await con.query(
      `UPDATE global_comment SET c_status = '1' WHERE id = '${payload.id}';`
    );
    _final_p = row;
  } catch (e) {
    _final_p = 'Bug';
