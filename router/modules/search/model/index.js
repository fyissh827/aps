...
try {
 const [row, fields] = await con.query(`SELECT subject.id, subject.type, subject.created_at, subject.name, subject.meaning, subject.file, subject.file_type,  users.first_name, users.last_name, users.profilepic, users.u_status, users.username  FROM subject , users  WHERE subject.user_id = users.id  AND subject.name LIKE "${ '%' + payload._h + '%'}" AND users.u_status = "0" ${p} ORDER BY  subject.id DESC LIMIT  ${payload.length}
   ` );
       _final = row;
...
try {
 const [row, fields] = await con.query(`SELECT id, first_name, last_name, profilepic, username, role, address  FROM users WHERE ((first_name LIKE "${ '%' + f + '%'}" OR last_name LIKE "${'%' + e + '%'}") AND u_status = '0' AND NOT id = "${payload._m}") OR ((last_name LIKE "${'%' + f + '%'}" OR first_name LIKE "${'%' + e + '%'}") AND u_status = '0') ${p} AND NOT id = "${payload._m}"  ORDER BY id LIMIT ${payload.length}` );
       _final = row;
...
try {
 const [row, fields] = await con.execute(`SELECT grewtales.id,grewtales.primitive_id,grewtales.g_status,grewtales.privacy,grewtales.user_id,CASE WHEN user_id='${payload._m}'THEN'YOU' ELSE'Another ' END AS who,grewtales.primitive_id,grewtales.w_n, grewtales.type, grewtales.content,grewtales.created_at,grewtales_file.file,grewtales_file.file_type, grews.title FROM grewtales , grewtales_file, users, grews WHERE grewtales.id=grewtales_file.parent   AND grewtales.primitive_id = grews.primitive_id AND users.id = grewtales.user_id ${p} AND (grews.title LIKE "${ '%' + payload._h + '%'}" AND NOT grewtales.privacy = '2' AND grewtales.g_status= '0'  OR grewtales.w_n LIKE "${ '%' + payload._h + '%'}" AND NOT grewtales.privacy = '2' AND grewtales.g_status= '0')   ORDER BY id DESC LIMIT  ${payload.length}`);
  console.log(fields);
 output = row;
 }
	  catch(e){
