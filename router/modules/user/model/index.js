...
async function subject(payload) {
  ...
  try {
    const [row, fields] = await con.execute(
      `SELECT subject.id, subject.name, subject.meaning, subject.type, subject.file, subject.file_type, subject.created_at, users.username FROM users INNER JOIN subject ON subject.user_id = users.id WHERE username = '${payload._m}' ORDER BY subject.id DESC`
    );
    _final = row;
  } catch (e) {
    _final = 'Bug';
...
async function grewtale_delete(payload) {
  ...
  try {
    const [row, fields] = await con.execute(
      `UPDATE  grewtales SET g_status = '1' , suspended = now() WHERE id = '${payload.id}'`
    );
    _final_p = row;
  } catch (e) {
    _final_p = 'Bug';
  ...
  if (_final_p == undefined) {
    ...
    //sync to dbs
    syncEmitter.emit('grewtaleDelete', payload);
    //end
    output = 'Deleted';
  }
  await _output.response(output);
