...
async function past_delete(payload) {
  ...
  try {
    const [row, fields] = await con.query(
      `DELETE FROM ${payload.type} WHERE id = '${payload.id}'`
    );
    _final = row;
  } catch (e) {
    console.log(e);
  ...
  if (_final == undefined || _final === 'Bug') {
    ...
    output = 'Nothing';
     ...
     //sync to dbs
        syncEmitter.emit(`${payload.type}Delete`, payload);
        //end
    output = 'Deleted';
  }
  await _output.response(output);
...
async function subject_delete(payload) {
  var _final = [];
  var output = [];

  try {
    const [row, fields] = await con.query(
      `DELETE FROM subject WHERE id = ${con.escape(payload.id)}`
    );
    _final = row;
  } catch (e) {
    console.log(e);
  ...
  if (_final == undefined || _final === 'Bug') {
    ...
    output = 'Nothing';
     ...
     //sync to dbs 
          syncEmitter.emit('wordDelete', payload);
        //end
    output = 'Deleted';
  }
  await _output.response(output);
...
module.exports = { past, present, future, past_delete, subject_delete };
