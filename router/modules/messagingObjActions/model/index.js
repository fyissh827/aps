...
const { user_messaging_requests } = require('../../models/index.js');
...
async function acceptMessagingRequest(payload) {
  ...
  try {
    const [r, s] = await con.query(
      `UPDATE user_messaging_requests SET status= "2" WHERE id = ${payload.id}`
    );
    _final = r;
  } catch (e) {
    _final = 'Bug';
...
async function deleteMessagingRequest(payload) {
  ...
  try {
    const [r, s] = await con.query(
      `DELETE FROM user_messaging_requests  WHERE id = ${payload.dataId}`
    );
    _final = r;
  } catch (e) {
    _final = 'Bug';
...
async function sendMessagingRequest(payload) {
  ...
  try {
    const [r, s] =
      await con.query(`SELECT *  FROM connectors  WHERE ((user_1 = ${payload.userTo} && user_2 =${payload.userFrom}) || 
  ...
  if (_final == undefined || _final === 'Bug') {
    ...
    try {
      const [r, s] =
        await con.query(`SELECT *  FROM user_messaging_requests  WHERE (user_to = ${payload.userTo} && user_from =${payload.userFrom}) || 
