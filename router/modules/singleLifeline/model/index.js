...
try {
 ...
 const [row, fields] = await con.query(`SELECT ${payload.obj}.id, ${payload.obj}.name, ${payload.obj}.type,  ${payload.obj}.file, ${payload.obj}.file_type, ${payload.addedQuery} ${payload.obj}.created_at
 FROM ${payload.obj}  WHERE id = ${payload.objId}`);
	  ...
	  if(_final == undefined){
	   ...
	   try{
		const [_row, _fields] = await con.query(`SELECT status as messagingStatus , id as messagingStatusId  from user_messaging_requests  WHERE user_from = ${payload.user1} AND user_to = ${payload.user2} OR user_from = ${payload.user2} AND user_to = ${payload.user1}`);
