...
try {
 const [row, fields] = await con.query(`UPDATE users SET alertSeen = ${payload.seen}  WHERE id = ${payload.id}`);
       _final = row;
...
try {
 const [row, fields] = await con.query(`INSERT INTO grewtales_file (type, parent, file, file_type, created_at ) VALUES ('0', '${payload_2.id}', '${payload_2.obj}', '${payload_2.file_type}',  now())` );
       _final = row;
