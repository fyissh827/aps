...
async function update(payload) {
  ...
  try {
    const [row, fields] = await con.query(
      `UPDATE users SET profilepic = '${payload.profilepic}', profile_update = now()   WHERE id = '${payload.user_id}'`
    );
    _final = row;
  } catch (e) {
    _final = 'Bug';
  ...
  if (_final == undefined || _final === 'Bug') {
    ...
    output = 'Nothing';
     ...
     syncEmitter.emit('profileUpdateProfilepic',  d);
...
const deleted = async (payload) => {
  ...
  try {
    const [row, fields] = await con.execute(
      `UPDATE  users SET profilepic = '1.svg' WHERE id = '${payload._m}'`
    );
    _final = row;
  } catch (e) {
    _final = 'Bug';
