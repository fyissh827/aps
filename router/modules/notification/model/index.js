...
try {
 const [row, fields] = await con.query(`UPDATE connectors SET notification_status  = '1'  WHERE id = '${payload.id}'` );
       _final = row;
...
try {
 const [row, fields] = await con.execute(`UPDATE  connectors SET notification_seen = notification_seen + 1  WHERE (user_2 = '${payload.user_id}' OR user_1 = '${payload.user_id}') AND notification_seen <  '2' AND  id IN (${payload.from})`);
  _final = row;
