...
const { PrivateMessage } = require('../../models/index.js');
...
async function _get_count(payload) {
  ...
  try {
    const [row, err] = await con.query(
      `SELECT COUNT(id) private_message WHERE  reciever = '${payload.user1}' AND seen = '0'`
    );
    _final_p = row;
  } catch (e) {
    _final_p = 'Bug';
...
async function _getsingle(payload) {
  ...
  try {
    const [row, err] = await con.query(
      `SELECT rp.id AS r_id, rp.message AS r_message, rp.file AS r_file, rp.file_type AS r_file_type, rp.seen AS r_seen, pm.id, pm.sender, CASE WHEN pm.sender = '${payload.user1}' THEN 'true' else 'false' END AS fromSelf , pm.reciever, pm.message, pm.accelerator, pm.file, pm.file_type, pm.status , pm.seen, pm.url,pm.messageType, pm.objType, pm.objId, pm.created_at FROM private_message pm LEFT JOIN private_message rp ON rp.id = pm.accelerator WHERE pm.id = '${payload.id}' AND pm.status = '0'`
    );
    _final_p = row;
  } catch (e) {
    _final_p = e;
...
async function _delete(payload) {
  ...
  try {
    const [row, err] = await con.query(
      `UPDATE private_message SET status = '1' WHERE id = '${payload.id}';`
    );
    _final_p = row;
  } catch (e) {
    _final_p = 'Bug';
...
async function _getusernew(payload) {
  ...
  try {
    const [row, err] = await con.query(
      `SELECT id, first_name, last_name, username, profilepic, online, socket_id, online_at FROM users WHERE id = '${payload.id}' AND u_status = '0';`
    );
    _final_p = row;
  } catch (e) {
    _final_p = 'Bug';
