...
try {
 const [row, err] = await con.query(`UPDATE users SET online = '1', socket_id = '${payload.socket_id}' WHERE id = '${payload.id}'`);
  _final_p = row;
}catch(e){
		_final_p = 'Bug'
	  }
...
try {
 const [row, err] = await con.query(`UPDATE users SET online = '0', socket_id = 'free' WHERE id = '${payload.id}'`);
  _final_p = row;
}catch(e){
		_final_p = 'Bug'
	  }
