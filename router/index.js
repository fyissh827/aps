...
const { otpCheckSignup } = require('./modules/otpCheck/otpCheckSignup.js');
const { checkEmail } = require('./modules/accountRecovery/checkEmail.js');
const { checkOtp } = require('./modules/accountRecovery/checkOtp.js');
const { setPassword } = require('./modules/accountRecovery/setPassword.js');
const { requestEmail } = require('./modules/accountRecovery/requestEmail.js');
const { requestEmailVerification } = require('./modules/registerVerification/requestEmail.js');
const { checkOtpRegisterVerification } = require('./modules/registerVerification/checkOtp.js');


//acess token
 const {acessToken} = require('./modules/accessToken/getToken.js');
const { logout } = require('./modules/accessToken/logout.js');
...
router
  .route('/edit/profile')
  .post(userMiddleware.isLoggedIn, edit.edit_profile);
router
  .route('/edit/address')
  .post(userMiddleware.isLoggedIn, editaddress.edit_address);
...
router
  ...
  .get(userMiddleware.isLoggedIn, me_address.me);

// userprofile photo update +m

router.use(
  '/api/user/cus_profile/v1/photo',
  express.static('media/user/image')
);
router.use(
  '/api/user/cus_profile/v2/photo',
  express.static('media/user/images')
);
router
  .route('/upload/profilepic')
  .post(pic_update.profilepic);
router
  .route('/remove/profilepic')
  .post(userMiddleware.isLoggedIn, pic_remove.remove);

//lifeline&sub +m

router.route('/insert/past').post(userMiddleware.isLoggedIn, past.past);
router
  .route('/insert/present')
  .post(userMiddleware.isLoggedIn, present.present);
router.route('/insert/future').post(userMiddleware.isLoggedIn, future.future);
router
  .route('/insert/subject')
  .post(userMiddleware.isLoggedIn, subject.subject);
...
router
  .route('/search/history/post')
  .post(userMiddleware.isLoggedIn, search_history.search_history);
router
  .route('/search/history/get')
  .get(userMiddleware.isLoggedIn, search_get.search_get);

// searching +m

router.route('/search/users').post(userMiddleware.isLoggedIn, user_s.user);
router
  .route('/search/grewtale')
  .post(userMiddleware.isLoggedIn, grewtale_s.grewtale);
router.route('/search/past').post(userMiddleware.isLoggedIn, past_s.past);
router
  .route('/search/present')
  .post(userMiddleware.isLoggedIn, present_s.present);
router.route('/search/future').post(userMiddleware.isLoggedIn, future_s.future);
router
  .route('/search/subject')
  .post(userMiddleware.isLoggedIn, subject_s.subject);

// posting +m

router.use(
  '/api/grewtale/media/v1/photo',
  express.static('media/grewtales/images')
);
...
router.use(
  '/api/comment/media/v1/photo',
  express.static('media/comment/images')
);
...
router
  .route('/testing/files')
  .post(userMiddleware.isLoggedIn, grewtale.grewtale);
router.route('/testing/image').post(multi.upload.array('photos', 30));

//home +m

router
  .route('/connectors/get')
  .post(userMiddleware.isLoggedIn, connectors_get.connectors_get);
router.route('/get/grewtale').post(userMiddleware.isLoggedIn, get_g.grewtale);
router
  .route('/get/grewtale/connection')
  .post(userMiddleware.isLoggedIn, get_g_connection.grewtale);
router.route('/get/last/post').post(userMiddleware.isLoggedIn, post.grewtale);
router
  .route('/get/grewtale/p')
  .post(userMiddleware.isLoggedIn, id_get.grewtale);
...
router
  .route('/get/present')
  .post(userMiddleware.isLoggedIn, get_present.get_present);
router
  .route('/get/future')
  .post(userMiddleware.isLoggedIn, get_future.get_future);
router
  .route('/get/subject')
  .post(userMiddleware.isLoggedIn, get_subject.get_subject);
router
  .route('/grewtale/delete')
  .post(userMiddleware.isLoggedIn, grewtale_delete.grewtale_delete);
router
  .route('/past/delete')
  .post(userMiddleware.isLoggedIn, past_delete.past_delete);
router
  .route('/present/delete')
  .post(userMiddleware.isLoggedIn, present_delete.present_delete);
router
  .route('/future/delete')
  .post(userMiddleware.isLoggedIn, future_delete.future_delete);
router
  .route('/subject/delete')
  .post(userMiddleware.isLoggedIn, subject_delete.subject_delete);

//plotters route +m

router
  .route('/plotter/inside')
  .post(userMiddleware.isLoggedIn, plotter_insert.plotter_insert);
router
  .route('/plotter/remove')
  .post(userMiddleware.isLoggedIn, plotter_delete.plotter_delete);

//plotters lifeline route + m

router
  .route('/past/plotter/inside')
  .post(userMiddleware.isLoggedIn, plotter_past_insert.past_plotter_insert);
router
  .route('/past/plotter/remove')
  .post(userMiddleware.isLoggedIn, plotter_past_delete.past_plotter_delete);

router
  .route('/present/plotter/inside')
  .post(
    userMiddleware.isLoggedIn,
    plotter_present_insert.present_plotter_insert
  );
router
  .route('/present/plotter/remove')
  .post(
    userMiddleware.isLoggedIn,
    plotter_present_delete.present_plotter_delete
  );

router
  .route('/future/plotter/inside')
  .post(userMiddleware.isLoggedIn, plotter_future_insert.future_plotter_insert);
router
  .route('/future/plotter/remove')
  .post(userMiddleware.isLoggedIn, plotter_future_delete.future_plotter_delete);

//connectors + m

router
  .route('/connectors/request')
  .post(userMiddleware.isLoggedIn, connectors_request.connectors_request);
router
  .route('/connectors/accept')
  .post(userMiddleware.isLoggedIn, connectors_accept.connectors_accept);
router
  .route('/connectors/delete')
  .post(userMiddleware.isLoggedIn, connectors_delete.connectors_delete);

//Report

router
  .route('/report/grewtale')
  .post(userMiddleware.isLoggedIn, report_grewtale.report_grewtale);
router
  .route('/report/comment')
  .post(userMiddleware.isLoggedIn, report_comment.report_comment);

//notification + m

router
  .route('/notification/get1')
  .post(userMiddleware.isLoggedIn, notification_get.notification_get);

router
  .route('/notification/remove1')
  .post(userMiddleware.isLoggedIn, connector_stop.connector_stop);
router
  .route('/notification/count')
  .post(userMiddleware.isLoggedIn, notification_count.notification_count);
router
  .route('/notification/count/destroyed')
  .post(
    userMiddleware.isLoggedIn,
    notification_count_destroyed.notification_count_destroyed
  );
...
router
  .route('/alert/get')
  .post(userMiddleware.isLoggedIn, alerts_get.alerts_get);
router
  .route('/alert/seen/set')
  .post(userMiddleware.isLoggedIn, alert_seen_set.alert_seen_set);
...
router
  .route('/chat/message/get')
  .post(userMiddleware.isLoggedIn, chat_get.chat_get);
router.route('/chat/message/count').post(chat_count.chat_count);
router
  .route('/chat/message/user')
  .post(userMiddleware.isLoggedIn, chat_user_get.chat_user_get);
router
  .route('/chat/message/user/new')
  .post(userMiddleware.isLoggedIn, chat_user_get_new.chat_user_get_new);
...
router
  ...
  .post(userMiddleware.isLoggedIn, st_privacy.privacy);
router
  .route('/setting/messaging')
  .post(userMiddleware.isLoggedIn, st_messaging.messaging);
router
  .route('/setting/messaging/rules')
  .post(userMiddleware.isLoggedIn, st_messaging_rules.messaging_rules);
router
  .route('/setting/messaging/rules/delete')
  .post(
    userMiddleware.isLoggedIn,
    st_messaging_rules_delete.messaging_rules_delete
  );
router
  .route('/setting/search')
  .post(userMiddleware.isLoggedIn, st_search.search);
router.route('/setting/sound').post(userMiddleware.isLoggedIn, st_sound.sound);
router
  .route('/setting/vibration')
  .post(userMiddleware.isLoggedIn, st_vibration.vibration);
router
  .route('/setting/username')
  .post(userMiddleware.isLoggedIn, st_username.username);
...
router
  .route('/setting/delete/search')
  .post(userMiddleware.isLoggedIn, delete_search._delete);
router
  .route('/setting/deactivate/account')
  .post(userMiddleware.isLoggedIn, deactivate_user._deactivate);
router
  .route('/setting/delete/account')
  .post(userMiddleware.isLoggedIn, delete_user._delete);
router.route('/setting/get').get(userMiddleware.isLoggedIn, get_st.get);
router
  .route('/setting/password/check')
  .post(userMiddleware.isLoggedIn, password_check.password_check);
router
  .route('/setting/password/update')
  .post(userMiddleware.isLoggedIn, password_update.password_update);
...
router
  .route('/messagingRequest/accept')
  .post(messagingRequestAccept.messagingRequestAccept);
router
  .route('/messagingRequest/delete')
  .post(messagingRequestDelete.messagingRequestDelete);
router
  .route('/messagingRequest/send')
  .post(userMiddleware.isLoggedIn, messagingRequestSend.messagingRequestSend);
...
router
  .route('/lifeline/single')
  .post(userMiddleware.isLoggedIn, getLifelineSingle.getLifelineSingle);
...
//otpCheck
router.route('/otpcheck/signup').post(otpCheckSignup);


//recovery

router.route('/accountrecovery/email').post(checkEmail);
router.route('/accountrecovery/otp').post(checkOtp);
router.route('/accountrecovery/password').post(setPassword);
router.route('/accountrecovery/requestemail').post(requestEmail);


//register verification

router.route('/registration/requestemail').post(requestEmailVerification);
router.route('/registration/verification').post(checkOtpRegisterVerification);
